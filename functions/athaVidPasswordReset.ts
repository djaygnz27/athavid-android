...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (action === "request") {
    ...
    const gmailToken = Deno.env.get("GMAIL_TOKEN");
    if (!gmailToken) {
...
<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#0f0f1a;padding:32px 24px;border-radius:20px;color:#fff;text-align:center;">
  ...
  if (action === "verify") {
    ...
    if (Date.now() > parseInt(record.expiry)) return new Response(JSON.stringify({ error: "Code has expired. Please request a new one." }), { status: 400, headers: cors });

    return new Response(JSON.stringify({ ok: true }), { headers: cors });
  }

  // ── Step 3: Set new password via Base44 admin ──────────────────────
  if (action === "reset") {
    ...
    // Use Base44 admin endpoint
    const adminRes = await fetch(`https://app.base44.com/api/apps/${appId}/auth/admin/set-password`, {
      method: "POST",
    ...
    if (!adminRes.ok) {
      ...
      const altRes = await fetch(`https://sachi-c7f0261c.base44.app/api/apps/${appId}/auth/admin/update-user`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": Deno.env.get("BASE44_API_KEY") || "" },
        body: JSON.stringify({ user_id: userId, password: new_password }),
      });
      if (!altRes.ok) {
        return new Response(JSON.stringify({ error: "Could not update password. Please contact support." }), { status: 500, headers: cors });
      }
    }
