...
function StatCard({ title, value, sub, color, icon }) {
  return (
    <div style={{
      ...
      <div style={{ color: COLORS.textMuted, fontSize: 12, marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 }}>{title}</div>
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ color, fontSize: 13, fontWeight: 700 }}>{value.toLocaleString()}</span>
      </div>
...
function buildWeeks(allVideos, allComments) {
  ...
  while (cursor <= thisWeekStart) {
    ...
    const cmts = allComments.filter(c => {
...
export default function Dashboard() {
  ...
  if (!currentWeek) {
    return (
      <div style={{ background: COLORS.navy, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ color: COLORS.textMuted, fontSize: 16 }}>No data yet. Upload some videos to get started!</div>
      </div>
    );
  }

  const wVideos = currentWeek.videos;
  const wComments = currentWeek.comments;
  ...
  const newVideos = wVideos.length;
  ...
  return (
    <div style={{ background: COLORS.navy, minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "#fff", padding: "0 0 60px" }}>
      {/* Header */}
      <div style={{
        background: "#0D0E1F",
        borderBottom: `1px solid ${COLORS.border}`,
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <StatCard title="New Videos" value={newVideos} color={COLORS.gold} icon="🎬" />
        </div>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          ...
          <StatCard title="Total Views" value={combinedVideos.reduce((s, v) => s + (v.views_count || 0), 0).toLocaleString()} color={COLORS.blue} icon="👁" />
          <StatCard title="Total Likes" value={combinedVideos.reduce((s, v) => s + (v.likes_count || 0), 0).toLocaleString()} color={COLORS.coral} icon="❤️" />
        ...
        <div style={{ background: COLORS.cardBg, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: "20px 24px" }}>
          ...
          <div style={{ marginTop: 16, color: COLORS.textMuted, fontSize: 12 }}>
            {activeTesters} active · {invitedTesters} pending invite · {betaTesters.length} total registered
          </div>
        </div>
