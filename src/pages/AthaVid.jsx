...
// ── Sign Up / Login Wall (shown only when user tries to upload) ───────────────
function AuthWall({ onClose, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signup") {
        await base44.auth.signUp({ email, password, full_name: name || email.split("@")[0] });
      } else {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:3000, display:"flex", alignItems:"flex-end" }}>
      ...
      <div style={{ position:"relative", width:"100%", maxWidth:480, margin:"0 auto", background:"#0f0f1a", borderRadius:"24px 24px 0 0", padding:"24px 24px 40px", zIndex:3001 }}>
        ...
        <button onClick={submit} disabled={loading}
          style={{ width:"100%", padding:"14px", background:"linear-gradient(135deg,#ff6b6b,#ff8e53)", border:"none", borderRadius:14, color:"#fff", fontWeight:800, fontSize:16, cursor:"pointer", opacity: loading ? 0.7 : 1 }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"0 16px 8px" }}>
          ...
          {comments.map(c => (
            <div key={c.id} style={{ display:"flex", gap:10, marginBottom:16 }}>
              <img src={c.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${c.username}`} style={{ width:36, height:36, borderRadius:"50%", border:"2px solid rgba(108,99,255,0.3)" }} />
              <div>
...
function UploadModal({ currentUser, onClose, onUploaded }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:2000, display:"flex", alignItems:"flex-end" }}>
      <div onClick={onClose} style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.85)" }} />
      <div style={{ position:"relative", width:"100%", maxWidth:480, margin:"0 auto", background:"#0f0f1a", borderRadius:"24px 24px 0 0", padding:"24px 24px 40px", zIndex:2001 }}>
...
export default function AthaVidApp() {
  ...
  // Try to get logged-in user — but don't block the app if not logged in
  useEffect(() => {
  ...
  return (
    <div style={{ background:"#000", minHeight:"100vh", maxWidth:480, margin:"0 auto", position:"relative", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", overflow:"hidden" }}>
        ...
        <div style={{ paddingTop:70, paddingBottom:80, minHeight:"100vh", background:"#0a0a14" }}>
          {!currentUser ? (
            <div style={{ textAlign:"center", padding:60 }}>
              ...
              <div style={{ color:"#666", fontSize:14, marginBottom:24 }}>Sign up to post videos and build your profile</div>
      ...
      {/* Bottom Nav */}
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:"rgba(8,8,16,0.96)", backdropFilter:"blur(20px)", borderTop:"1px solid rgba(255,255,255,0.06)", display:"flex", zIndex:200, paddingBottom:"env(safe-area-inset-bottom)" }}>
        {[
      ...
      {/* Modals */}
      {commentVideo && (
        <CommentSheet video={commentVideo} currentUser={currentUser}
          onClose={() => setCommentVideo(null)}
          onCommentPosted={handleCommentCount}
          onNeedAuth={() => { setCommentVideo(null); setShowAuth(true); }} />
      )}
      {showUpload && currentUser && (
        <UploadModal currentUser={currentUser} onClose={() => setShowUpload(false)} onUploaded={loadVideos} />
      )}
      {showAuth && (
        <AuthWall onClose={() => setShowAuth(false)} onSuccess={(user) => { setCurrentUser(user); setShowAuth(false); }} />
      )}
    </div>
  );
}
