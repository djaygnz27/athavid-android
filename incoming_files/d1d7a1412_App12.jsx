...
// ── Module-level mute store (replaces window.__sachiMuted) ───────────────────
...
// Module-level cache so concurrent upload calls share one in-flight geolocation request
...
async function getPostLocation() {
  ...
  _locationPromise = _fetchPostLocation().finally(() => { _locationPromise = null; });
...
function getStateAbbr(state, countryCode) {
  ...
  // For other countries, return state as-is
  return state;
}
...
async function captureThumbnail(file) {
  ...
  // Use a proper async function instead of new Promise(async) to avoid swallowed rejections
  ...
  const CommentRow = ({ c, isReply=false, parentId=null }) => {
    ...
    const saveEdit = async () => {
      ...
      try {
        ...
        setEditing(false);
      } catch(e) { console.error("Could not save edit:", e); }
  ...
  const uploadLive = async () => {
    try {
      ...
      try {
        ...
        if (thumbBlob) {
          thumbUrl = thumbBlob; // captureThumbnail now returns the URL directly
        }
      } catch(_) {}
...
function VideoCard({ video, currentUser, onCommentOpen, onLike, onView, onNeedAuth, onDelete, onProfileOpen, followedUserIds, onFollowChange, onShareCount, onBookmark, blockedIds }) {
  ...
  const [muted, _setMutedLocal] = useState(() => getMuted());
  const setMuted = (val) => {
  ...
  const photoUrls = video.is_photo
    ? (safeParsePhotoUrls(video.photo_urls) || (video.video_url ? [video.video_url] : null))
    : null;
  ...
  useEffect(() => {
    ...
    if (soundRef.current) {
      if (getMuted()) { soundRef.current.pause(); }
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        // If metadata not yet loaded, wait for it before playing
        if (el.readyState < 1) {
          el.onloadedmetadata = () => { tryPlay(); el.onloadedmetadata = null; };
        } else {
          tryPlay();
        }
        setShowUI(true);
  ...
  const confirmDelete = async () => {
    ...
    } catch(err) { console.error("Failed to delete:", err); alert("Failed to delete. Try again."); }
  };

  const tap = (fn) => (e) => { e.stopPropagation(); fn(); };

  return (
    <div style={{ position:"relative", width:"100%", height:"100svh", background:"#0B0C1A", flexShrink:0, scrollSnapAlign:"start", scrollSnapStop:"always" }}>
      ...
      {photoUrls ? (
        ...
        return (
          <div style={{ position:"relative", width:"100%", height:"100%", background:"#111", overflow:"hidden" }}>
            {/* Thumbnail shown as background layer — eliminates black screen while video loads.
                Fades out once video starts playing via the playing state. */}
            {video.thumbnail_url && !/\.(mp4|mov|webm|avi|mkv|m4v)(\?|$)/i.test(video.thumbnail_url) ? (
              ...
              <div style={{ position:"absolute", inset:0, zIndex:1, background:"linear-gradient(180deg,#1a1a2e,#0B0C1A)", opacity: playing ? 0 : 1, transition:"opacity 0.3s ease", pointerEvents:"none" }} />
            )}
            <video ref={videoRef}
      ...
      <div style={{ position:"absolute", bottom:"calc(80px + env(safe-area-inset-bottom, 0px))", left:16, right:16, zIndex:500, transition:"opacity 0.4s ease", opacity: (showUI || !!photoUrls) ? 1 : 0, pointerEvents: (showUI || !!photoUrls) ? "auto" : "none", visibility: (showUI || !!photoUrls) ? "visible" : "hidden" }}>
      ...
      <div style={{ position:"absolute", right:12, bottom:"calc(80px + env(safe-area-inset-bottom, 0px))", display:"flex", flexDirection:"column", alignItems:"center", gap:10, zIndex: 500, transition:"opacity 0.4s ease", opacity: (showUI || !!photoUrls) ? 1 : 0, pointerEvents: (showUI || !!photoUrls) ? "auto" : "none", visibility: (showUI || !!photoUrls) ? "visible" : "hidden" }}>
...
function UserProfileSheet({ userId, username, currentUser, onClose }) {
  ...
  React.useEffect(() => {
    setLoading(true);
    Promise.all([
      request("GET", `${APP_BASE}/entities/AthaVidUser?limit=200`).catch(() => null),
      videos.byUser(userId).catch(() => []),
      // Live follower count: how many people follow this profile
      request("GET", `${APP_BASE}/entities/Follow?following_id=${userId}&limit=500`).catch(() => null),
      // Live following count: how many people this profile follows
      request("GET", `${APP_BASE}/entities/Follow?follower_id=${userId}&limit=500`).catch(() => null),
  ...
  const doFollow = async () => {
    ...
    try {
      ...
      try {
        const myFollowingRes = await request("GET", `${APP_BASE}/entities/Follow?follower_id=${currentUser.id}&limit=500`);
...
function VideoManageGrid({ videos: vids, onRefresh }) {
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      setEditVideo(null);
      onRefresh();
    } catch(e) { console.error("Save failed:", e); alert("Could not save changes. Please try again."); }
    finally { setSaving(false); }
  };
...
function PodcastPage({ currentUser, onNeedAuth }) {
  ...
  const loadPodcasts = async () => {
    ...
    try {
      const data = await request("GET", `${APP_BASE}/entities/SachiPodcast?status=Active`);
  ...
  const loadMyShows = async () => {
    if (!currentUser) return;
    try {
      const data = await request("GET", `${APP_BASE}/entities/SachiPodcast`);
                ...
                <>
                    ...
                    {editingStream ? (
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        <button onClick={async () => {
                          try {
                            await request("PATCH", `${APP_BASE}/entities/SachiPodcast/${selectedPodcast.id}`, { live_stream_url: newStreamUrl });
                  ...
                  <button onClick={async () => {
                    ...
                    try {
                      ...
                      try { await request("PATCH", `${APP_BASE}/entities/SachiPodcast/${selectedPodcast.id}`, { is_live:false, listener_count:0 }); } catch {}
                ...
                <>
                  ...
                  <button onClick={async () => {
                    ...
                    try {
                      ...
                      try { await request("PATCH", `${APP_BASE}/entities/SachiPodcast/${selectedPodcast.id}`, { is_live:true }); } catch {}
            ...
            {myShows.map(p => {
              ...
              return (
                <div key={p.id} onClick={async () => {
                  ...
                  setPodcastEpisodes([]);
                  try {
                    const res = await request("GET", `${APP_BASE}/entities/SachiPodcastEpisode?podcast_id=${encodeURIComponent(p.id)}&sort=-episode_number&limit=200`);
                    const items = Array.isArray(res) ? res : (res?.records || res?.items || []);
...
function NotificationsPanel({ currentUser, onClose, onNotifRead }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await request("GET", `${APP_BASE}/entities/SachiNotification?recipient_id=${currentUser.id}&limit=50&sort=-created_date`);
        const items = Array.isArray(res) ? res : (res?.records || res?.items || []);
        setNotifs(items);
        items.filter(n => !n.is_read).forEach(n =>
          request("PATCH", `${APP_BASE}/entities/SachiNotification/${n.id}`, { is_read: true }).catch(() => {})
        );
  ...
  if (activeThread) return (
    <div style={{ position:"fixed", inset:0, background:"#0B0C1A", zIndex:500, display:"flex", flexDirection:"column" }}>
      <div style={{ padding:"14px 16px", paddingTop:"calc(env(safe-area-inset-top,0px) + 14px)", borderBottom:"1px solid rgba(255,255,255,0.08)", display:"flex", alignItems:"center", gap:12, background:"rgba(14,14,28,0.98)", backdropFilter:"blur(20px)" }}>
        <button onClick={() => { if (fromProfile) { onClose(); } else { setActiveThread(null); setThreadMsgs([]); loadInbox(); } }} style={{ background:"none", border:"none", color:"#F5C842", cursor:"pointer", fontSize:20, padding:0 }}>←</button>
...
function AdminPanel({ currentUser }) {
  ...
  const updateFounderStatus = async (founder, status) => {
    try {
      await request("PUT", `${APP_BASE}/entities/FoundingCreator/${founder.id}`, { status, notes: founderNote || founder.notes });
  ...
  const toggleMature = async (video, reason) => {
    ...
    try {
      ...
      await request("PUT", `${APP_BASE}/entities/SachiVideo/${video.id}`, {
        is_mature: newMature,
  ...
  const flagAI = async (video) => {
    ...
    try {
      const newFlag = !video.is_ai_detected;
      await request("PUT", `${APP_BASE}/entities/SachiVideo/${video.id}`, { is_ai_detected: newFlag });
                ...
                {allVideos.filter(v => v.is_ai_detected && !v.is_approved).map(video => (
                  <div key={video.id} style={{ background:"rgba(255,149,0,0.08)", borderRadius:16, border:"2px solid rgba(255,149,0,0.5)", overflow:"hidden" }}>
                    ...
                    <div style={{ display:"flex", gap:0, borderTop:"1px solid rgba(255,255,255,0.05)" }}>
                      <button onClick={async () => { setSaving(video.id); await request("PUT", `${APP_BASE}/entities/SachiVideo/${video.id}`, { is_approved: true }); setAllVideos(p => p.map(v => v.id===video.id ? {...v, is_approved:true} : v)); setSaving(null); }}
...
function App() {
  ...
  React.useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await request("GET", `${APP_BASE}/entities/SachiNotification?recipient_id=${currentUser.id}&is_read=false&limit=50`);
        const items = Array.isArray(res) ? res : (res?.records || res?.items || []);
    ...
    const loadAvatar = async () => {
      if (currentUser) {
        ...
        try {
          ...
          const usersData = await request("GET", `${APP_BASE}/entities/AthaVidUser/?email=${encodeURIComponent(currentUser.email)}`);
  ...
  useEffect(() => {
    if (activeTab === "profile" && currentUser) {
      ...
      (async () => {
        try {
          const r1 = await request("GET", `${APP_BASE}/entities/Follow?following_id=${currentUser.id}&limit=500`).catch(()=>null);
          const r2 = await request("GET", `${APP_BASE}/entities/Follow?following_username=${encodeURIComponent(myUsername)}&limit=500`).catch(()=>null);
      ...
      (async () => {
        try {
          const r1 = await request("GET", `${APP_BASE}/entities/Follow?follower_id=${currentUser.id}&limit=500`).catch(()=>null);
          const r2 = await request("GET", `${APP_BASE}/entities/Follow?follower_username=${encodeURIComponent(myUsername)}&limit=500`).catch(()=>null);
          const all = [...(r1?.items||r1||[]), ...(r2?.items||r2||[])];
          const unique = [...new Map(all.map(f => [f.id, f])).values()];
  ...
  return (
    <div style={{ background:"#0B0C1A", minHeight:"100svh", maxWidth:480, margin:"0 auto", position:"relative", fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }}>
      ...
      {/* Feed */}
      {activeTab === "feed" && (
        <div key={feedKey} ref={el => { feedContainerRef.current = el; window.__sachiEl = el; }} style={{ height:"100svh", overflowY:"scroll", scrollSnapType:"y mandatory", isolation:"isolate", touchAction:"pan-y", paddingBottom:0 }}>
          ...
          <div style={{ background:"#1a1a2e", borderRadius:20, padding:24, width:"100%", maxWidth:420 }}
            ...
            <div style={{ display:"flex", gap:10, marginTop:14 }}>
              ...
              <button onClick={async () => {
                  ...
                  try {
                    await request("PATCH", `${APP_BASE}/auth/me`, { full_name: editProfileName.trim() });
                    setCurrentUser(u => ({ ...u, full_name: editProfileName.trim() }));
                    setShowEditProfile(false);
                  } catch(e) { console.error("Save failed:", e); alert("Could not save name. Please try again."); }
      ...
      {showAvatarPicker && <AvatarPickerModal currentAvatar={avatarUrl} onSelect={async (url) => {
        ...
        try {
          ...
          if (match) {
            if (url.startsWith("data:")) {
              // Real base64 data URI — upload it
              ...
              // Already a hosted URL (DiceBear, CDN) — update entity directly with the URL
              await request("PATCH", `${APP_BASE}/entities/AthaVidUser/${match.id}/`, { avatar_url: url });
            }
          }
        ...
        try {
          await request("PUT", `${APP_BASE}/auth/me`, { avatar_url: url });
        ...
        try {
          const vidsData = await request("GET", `${APP_BASE}/entities/SachiVideo/?username=${encodeURIComponent(currentUser.username || currentUser.email?.split("@")[0])}&limit=200`);
          const vids = Array.isArray(vidsData) ? vidsData : (vidsData?.items || vidsData?.records || []);
          await Promise.all(vids.map(v => request("PATCH", `${APP_BASE}/entities/SachiVideo/${v.id}/`, { avatar_url: url })));
