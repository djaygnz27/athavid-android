// Creates a Cloudflare Stream live input with WebRTC publish + play URLs
  ...
  try {
    const cfRes = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT}/stream/live_inputs`,
      {
        ...
        body: JSON.stringify({
          meta: { name: `sachi-live-${username || "host"}-${Date.now()}` },
    ...
    if (!cfData.success) return res.status(500).json({ error: "Cloudflare error", details: cfData.errors });

    const input = cfData.result;
    return res.status(200).json({
      success: true,
      cf_input_id: input.uid,
      webrtc_publish_url: input.webRTC?.url,       // host pushes here
      webrtc_play_url: input.webRTCPlayback?.url,  // viewers watch here
