...
function VideoCard({ video, currentUser, onCommentOpen, onLike, onView, onNeedAuth, onDelete, onProfileOpen, followedUserIds, onFollowChange, onShareCount, onBookmark, blockedIds }) {
  ...
  const photoUrls = video.is_photo && video.photo_urls ? safeParsePhotoUrls(video.photo_urls) : null;
...
function UserProfileSheet({ userId, username, currentUser, onClose }) {
  ...
  React.useEffect(() => {
    setLoading(true);
    Promise.all([
      request$1("GET", `/apps/${APP_ID$1}/entities/AthaVidUser?limit=200`).catch(() => null),
      videos.byUser(userId).catch(() => []),
      // Live follower count: how many people follow this profile
      request$1("GET", `/apps/${APP_ID$1}/entities/Follow?following_id=${userId}&limit=500`).catch(() => null),
      // Live following count: how many people this profile follows
      request$1("GET", `/apps/${APP_ID$1}/entities/Follow?follower_id=${userId}&limit=500`).catch(() => null)
  ...
  const doFollow = async () => {
    ...
    try {
      ...
      try {
        const myFollowingRes = await request$1("GET", `/apps/${APP_ID$1}/entities/Follow?follower_id=${currentUser.id}&limit=500`);
...
function PodcastPage({ currentUser, onNeedAuth }) {
  ...
  const loadPodcasts = async () => {
    ...
    try {
      const data = await request$1("GET", `/apps/${APP_ID$1}/entities/SachiPodcast?status=Active`);
  ...
  const loadMyShows = async () => {
    ...
    try {
      const data = await request$1("GET", `/apps/${APP_ID$1}/entities/SachiPodcast`);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await request$1("POST", `/apps/${APP_ID$1}/entities/SachiPodcast`, {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 14, marginBottom: 14 }, children: [
              ...
              editingStream ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 8 }, children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: async () => {
                  try {
                    await request$1("PATCH", `/apps/${APP_ID$1}/entities/SachiPodcast/${selectedPodcast.id}`, { live_stream_url: newStreamUrl });
              ...
              "button",
              {
                onClick: async () => {
                  ...
                  try {
                    ...
                    try {
                      await request$1("PATCH", `/apps/${APP_ID$1}/entities/SachiPodcast/${selectedPodcast.id}`, { is_live: false, listener_count: 0 });
                    } catch {
                    }
              ...
              "button",
              {
                onClick: async () => {
                  ...
                  try {
                    ...
                    try {
                      await request$1("PATCH", `/apps/${APP_ID$1}/entities/SachiPodcast/${selectedPodcast.id}`, { is_live: true });
                    } catch {
                    }
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              onClick: async () => {
                ...
                setPodcastEpisodes([]);
                try {
                  const res = await request$1("GET", `/apps/${APP_ID$1}/entities/SachiPodcastEpisode?podcast_id=${p2.id}&limit=100&sort=-episode_number`);
...
function AdminPanel({ currentUser }) {
  ...
  const loadAnalytics = async () => {
    ...
    try {
      ...
      while (uMore) {
        const uRes = await request$1("GET", `/apps/${APP_ID$1}/entities/AthaVidUser?limit=500&skip=${uSkip}&sort=-created_date`);
      ...
      while (lMore) {
        const lRes = await request$1("GET", `/apps/${APP_ID$1}/entities/User?limit=500&skip=${lSkip}&sort=-created_date`);
  ...
  const loadRegisteredUsers = async () => {
    ...
    try {
      ...
      while (hasMore) {
        const res = await request$1("GET", `/apps/${APP_ID$1}/entities/AthaVidUser?limit=500&skip=${skip}&sort=-created_date`);
      ...
      while (hasMore2) {
        const res2 = await request$1("GET", `/apps/${APP_ID$1}/entities/User?limit=500&skip=${skip2}&sort=-created_date`);
  ...
  const toggleMature = async (video, reason) => {
    ...
    try {
      ...
      await request$1("PUT", `/apps/${APP_ID$1}/entities/SachiVideo/${video.id}`, {
        is_mature: newMature,
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 0, borderTop: "1px solid rgba(255,255,255,0.05)" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                onClick: async () => {
                  setSaving(video.id);
                  await request$1("PUT", `/apps/${APP_ID$1}/entities/SachiVideo/${video.id}`, { is_approved: true });
...
function App() {
  ...
  reactExports.useEffect(() => {
    const loadAvatar = async () => {
      if (currentUser) {
        try {
          const usersData = await request$1("GET", `/apps/${APP_ID$1}/entities/AthaVidUser/?email=${encodeURIComponent(currentUser.email)}`);
  ...
  reactExports.useEffect(() => {
    ...
    if (activeTab === "profile" && currentUser) {
      ...
      (async () => {
        try {
          const r1 = await request$1("GET", `/apps/${APP_ID$1}/entities/Follow?following_id=${currentUser.id}&limit=500`).catch(() => null);
          const r2 = await request$1("GET", `/apps/${APP_ID$1}/entities/Follow?following_username=${encodeURIComponent(myUsername)}&limit=500`).catch(() => null);
      ...
      (async () => {
        try {
          const r1 = await request$1("GET", `/apps/${APP_ID$1}/entities/Follow?follower_id=${currentUser.id}&limit=500`).catch(() => null);
          const r2 = await request$1("GET", `/apps/${APP_ID$1}/entities/Follow?follower_username=${encodeURIComponent(myUsername)}&limit=500`).catch(() => null);
          const all = [...(r1 == null ? void 0 : r1.items) || r1 || [], ...(r2 == null ? void 0 : r2.items) || r2 || []];
          const unique = [...new Map(all.map((f2) => [f2.id, f2])).values()];
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { background: "#0B0C1A", minHeight: "100svh", maxWidth: 480, margin: "0 auto", position: "relative", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif" }, children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    onClick: async () => {
                      ...
                      try {
                        await request$1("PUT", `/apps/${APP_ID$1}/auth/me`, { full_name: editProfileName.trim() });
    ...
    showAvatarPicker && /* @__PURE__ */ jsxRuntimeExports.jsx(AvatarPickerModal, { currentAvatar: avatarUrl, onSelect: async (url) => {
      ...
      if (currentUser && !url.startsWith("data:")) {
        try {
          await request$1("PUT", `/apps/${APP_ID$1}/auth/me`, { avatar_url: url });
        } catch (e) {
        ...
        try {
          ...
          const match = users.find((u2) => u2.email === currentUser.email || u2.user_id === currentUser.id);
          if (match) await request$1("PATCH", `/apps/${APP_ID$1}/entities/AthaVidUser/${match.id}/`, { avatar_url: url });
        } catch (e) {
          console.warn("User entity update failed:", e);
        }
        try {
          const vidsData = await request$1("GET", `/apps/${APP_ID$1}/entities/SachiVideo/?created_by=${currentUser.id}&limit=200`);
          const vids = Array.isArray(vidsData) ? vidsData : (vidsData == null ? void 0 : vidsData.items) || (vidsData == null ? void 0 : vidsData.records) || [];
          await Promise.all(vids.map((v2) => request$1("PATCH", `/apps/${APP_ID$1}/entities/SachiVideo/${v2.id}/`, { avatar_url: url })));
...
function PodcastHost() {
  ...
  const [streamKey, setStreamKey] = reactExports.useState(null);
    ...
    try {
      const res = await fetch(`${SACHI_FN}/createLiveStream`, {
        ...
        body: JSON.stringify({
          ...
          host_username: selectedShow.host_username || (user == null ? void 0 : user.full_name) || "host"
        })
      });
      const data = await res.json();
      if (data.success) {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: s.body, children: [
      ...
      view === "manage" && selectedShow && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        ...
        selectedShow.is_live ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: s.liveCard, children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontWeight: 700, fontSize: 15, marginBottom: 14 }, children: "🔴 Go Live with OBS" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { background: "rgba(245,200,66,0.07)", border: "1px solid rgba(245,200,66,0.2)", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontWeight: 700, fontSize: 13, color: "#F5C842" }, children: "🎙️ OBS Stream Key" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(255,255,255,0.4)", fontSize: 11, marginTop: 2 }, children: "Use with OBS Studio to stream directly to Sachi" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: getStreamKey, disabled: loadingKey, style: { ...s.smallBtn("rgba(245,200,66,0.2)", "#F5C842"), fontSize: 11, padding: "6px 12px" }, children: loadingKey ? "Loading..." : streamKey ? "Refresh Key" : "Get Stream Key" })
            ] }),
            streamKey && showKey && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginTop: 4 }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: 8 }, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(255,255,255,0.4)", fontSize: 10, marginBottom: 3 }, children: "SERVER URL" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { flex: 1, background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: "7px 10px", fontFamily: "monospace", fontSize: 11, color: "#a78bfa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: rtmpUrl || "rtmps://live.cloudflare.com:443/live/" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      onClick: () => {
                      ...
                      style: { ...s.smallBtn("rgba(108,60,247,0.3)", "#a78bfa"), fontSize: 11, padding: "6px 10px", flexShrink: 0 },
                      children: "Copy"
                    }
                  )
                ] })
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(255,255,255,0.4)", fontSize: 10, marginBottom: 3 }, children: "STREAM KEY (keep private!)" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { flex: 1, background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: "7px 10px", fontFamily: "monospace", fontSize: 11, color: "#4ade80", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: streamKey }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      onClick: () => {
                      ...
                      style: { ...s.smallBtn("rgba(74,222,128,0.2)", "#4ade80"), fontSize: 11, padding: "6px 10px", flexShrink: 0 },
                      children: "Copy"
                    }
                  )
                ] })
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { color: "rgba(255,255,255,0.35)", fontSize: 11 }, children: "New to OBS? Download the setup guide" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "a",
                {
                  href: "https://base44.app/api/apps/69b2ee18a8e6fb58c7f0261c/files/mp/public/69b2ee18a8e6fb58c7f0261c/bbc3469b5_Sachi_OBS_Setup_Guide.pdf",
