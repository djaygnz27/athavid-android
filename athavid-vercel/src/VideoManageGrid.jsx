...
// Last verified working: 2026-06-29 (tap=play, long-press/⋮=edit/delete, 72px circles, 4 per row)
...
function VideoManageGrid({ videos: vids, onRefresh, currentUser }) {
  ...
  const handlePressEnd = (v, idx) => {
    clearTimeout(pressTimer.current);
    if (!didLongPress.current) setPlayerIndex(idx);
  };
  const handlePressCancel = () => {
    clearTimeout(pressTimer.current);
    didLongPress.current = true; // suppress tap
  };
  ...
  if (!vids || vids.length === 0) return (
    <div style={{ textAlign:"center", padding:40, color:"#555" }}>
      ...
      <div style={{ color:"rgba(255,255,255,0.4)", fontSize:14 }}>No videos yet — post something!</div>
    </div>
  );

  // ── Layout: 72px circles, 4 per row ──
  const CELL = 72;
  const GAP = 8;
  const PER_ROW = 4;
  ...
  return (
    <>
      ...
      <div style={{ paddingBottom:16, paddingTop:8 }}>
        {rows.map((row, ri) => (
          <div key={ri} style={{ display:"flex", justifyContent:"center", gap:GAP, marginBottom:GAP, paddingLeft:8, paddingRight:8 }}>
            {row.map((v, ci) => {
              ...
              return (
                <div key={v.id}
                  ...
                  onMouseUp={() => handlePressEnd(v, globalIdx)}
                  onMouseLeave={handlePressCancel}
                  onTouchStart={() => handlePressStart(v)}
                  onTouchEnd={() => handlePressEnd(v, globalIdx)}
                  ...
                  style={{
                    ...
                    boxShadow: isMedal
                      ...
                      : "0 0 8px rgba(245,200,66,0.25), 0 3px 8px rgba(0,0,0,0.5)",
                    background:"#111",
                  ...
                  {/* Overlay */}
                  <div style={{ position:"absolute", inset:0, background:"radial-gradient(circle at 50% 75%, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.05) 65%)", pointerEvents:"none" }} />
                  {/* Medal */}
                  {isMedal && <div style={{ position:"absolute", top:2, right:4, fontSize:12, lineHeight:1, pointerEvents:"none" }}>{medals[medalIdx]}</div>}
                  {/* Like count */}
                  {(v.likes_count||0) > 0 && (
                    <div style={{ position:"absolute", bottom:8, left:0, right:0, textAlign:"center",
                      color:"#fff", fontSize:8, fontWeight:800, textShadow:"0 1px 4px rgba(0,0,0,0.9)", pointerEvents:"none" }}>
                      ❤️ {v.likes_count}
                    </div>
                  )}
                  {/* ⋮ menu button — tap this to edit/delete */}
                  <button
                    onMouseDown={e => e.stopPropagation()}
                    onTouchStart={e => { e.stopPropagation(); setMenuVideo(v); }}
                    onClick={e => { e.stopPropagation(); setMenuVideo(v); }}
                    style={{ position:"absolute", top:4, left:4,
                      width:18, height:18, borderRadius:"50%", background:"rgba(0,0,0,0.65)",
        ...
        <div style={{ position:"fixed", inset:0, zIndex:9000, display:"flex", flexDirection:"column", justifyContent:"flex-end" }}
          ...
          <div style={{ background:"#1a1a2e", borderRadius:"20px 20px 0 0", padding:20, maxWidth:480, width:"100%", margin:"0 auto" }}
            ...
            <div style={{ display:"flex", gap:12, marginBottom:20, alignItems:"center" }}>
              <div style={{ width:54, height:54, background:"#111", borderRadius:"50%", overflow:"hidden", flexShrink:0, border:"2px solid rgba(245,200,66,0.4)" }}>
            ...
            <button onClick={() => { setPlayerIndex(vids.indexOf(menuVideo)); setMenuVideo(null); }}
              style={{ width:"100%", padding:"14px 0", background:"rgba(99,102,241,0.15)", border:"1px solid rgba(99,102,241,0.4)",
                borderRadius:12, color:"#a5b4fc", fontSize:15, fontWeight:600, cursor:"pointer", marginBottom:10,
          ...
          <div style={{ background:"#1a1a2e", borderRadius:20, padding:24, width:"100%", maxWidth:420 }}
            ...
            <div style={{ display:"flex", gap:10, marginTop:14 }}>
              <button onClick={() => setEditVideo(null)}
                style={{ flex:1, padding:"12px 0", background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, color:"#aaa", fontSize:14, cursor:"pointer" }}>Cancel</button>
          ...
          <div style={{ background:"#1a1a2e", borderRadius:20, padding:24, width:"100%", maxWidth:380, textAlign:"center" }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setConfirmDelete(null)}
                style={{ flex:1, padding:"13px 0", background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, color:"#aaa", fontSize:14, cursor:"pointer" }}>Cancel</button>
