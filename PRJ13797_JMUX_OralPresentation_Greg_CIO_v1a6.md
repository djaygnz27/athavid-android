...
The program is structured around four distinct workstreams, and all four are running concurrently. Let me walk through each one.

The first workstream is Engineering Services, led by Burns and McDonnell. B&M is responsible for all of the technical engineering work that happens before a single piece of equipment gets installed. That includes conducting site surveys at every substation — physically going on-site to assess the existing infrastructure, rack space, power availability, and cabling pathways. They produce rack elevation drawings, power designs, and cabling designs for each site. They also produce all the formal engineering documents — the IFR, which is Issued for Review, the IFC, which is Issued for Construction, and the As-Built documents that are filed once installation is complete. Nothing gets built until B&M produces and approves the IFC for that site. The engineering package is the gate that everything else flows through.

The second workstream is Procurement and Staging, managed jointly by Burns and McDonnell, Nokia, and Graybar. This workstream is responsible for getting all the right equipment to the right place at the right time. B&M creates the Bill of Materials for each site based on the engineering design. Nokia supplies the MPLS router equipment — the core of what we are installing at each substation. Graybar is our distribution partner handling the ancillary materials — things like patch panels, fiber cables, power equipment, and mounting kits. All of this equipment is procured, received, and staged — meaning it is physically prepared and verified before it goes to a substation. Staging happens at our Hawkeye trailer facility and it is a critical quality control step that ensures every component is accounted for and functional before it touches a live substation.

The third workstream is Construction and Installation Support, led by Burns and McDonnell and Hawkeye. Once the IFC is issued and equipment is staged, Hawkeye executes the physical installation at each site. Hawkeye handles shipment of equipment from staging to the substation, all racking and stacking, power connections, and fiber or copper cabling — all installed per the approved IFC document. Burns and McDonnell provides on-site coordination, technical support, and commissioning oversight throughout the construction process. B&M is there to ensure the installation matches the engineering design exactly and to troubleshoot any field conditions that differ from what was shown on the drawings. Hawkeye also continues conducting site surveys for each new site as IFCs are issued, so the construction planning stays ahead of the installation schedule.

The fourth workstream is Deployment and Application Migration, led by our PSEG team working with Burns and McDonnell. This is where the new Nokia MPLS infrastructure actually goes live. The PSEG and B&M team designs and configures all of the Layer 3 networks — the BGP routing, the IP addressing, the network policies. They pre-stage the application connectivity so that when we cut over from the legacy JMUX to the new Nokia nodes, the applications — SCADA, EMS, and all the operational systems that depend on substation communications — transition cleanly without disruption. And then they execute the actual production application migration, moving each application from the old JMUX circuits to the new MPLS network, site by site.

All four of those workstreams are running simultaneously, across three phases, at over 100 substations. That is what makes this program complex to manage — and it is why having clear sequencing, clear ownership, and clear engineering gates at each phase is critical to keeping the program on track."
